...
referencia de todos los componentes del sistema mecánico, en este caso el sensor lidar y el 
...
sistema mecánico es autocalibrado accionado por el switch, el sensor lidar es posicionado 
como muestra la Figura \ref{f:Rot3D}a, en esa posición se considera que el ángulo del 
motor paso a paso es 0\grad. Luego el sensor lidar se comienza a rotar hacia el lado 
derecho hasta que llega al ángulo $-88$\grad~ (ver Figura \ref{f:Rot3D}b). Una vez que
el motor paso a paso llega al ángulo $-88$\grad este comienza a girar hacia la izquierda 
...
lidar pueda tener un panorama más amplio para poder tomar mediciones dentro de un ambiente.
...
El mapa tridimensional se construye con las mediciones del sensor lidar y los ángulos del motor 
paso a paso. Para esto primero se convierte los datos del sensor lidar, es decir hacer una 
...
\ref{f:lidarPlot}b se puede ver la conversión a coordenadas cartesianas. Las mediciones 
mostradas en la Figura \ref{f:lidarPlot} fueron realizadas dentro de un cuarto, esto sirve para 
explicar la diferencia entre las coordenadas.Las ecuaciones utilizadas son:
\begin{align*}
	x &= rcos(\theta_{L}), \\
	y &= rsen(\theta_{L}),
\end{align*}
